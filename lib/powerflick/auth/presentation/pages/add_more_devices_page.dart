...
class AddMoreDevicesPage extends ConsumerStatefulWidget {
  const AddMoreDevicesPage({super.key});

  @override
  ConsumerState<AddMoreDevicesPage> createState() => _AddMoreDevicesPageState();
}

class _AddMoreDevicesPageState extends ConsumerState<AddMoreDevicesPage> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _fetchRoomsAndDevices(); // Fetch both rooms and existing devices
  }
  ...
  Future<void> _fetchRoomsAndDevices() async {
    final user = Supabase.instance.client.auth.currentUser;
    if (user == null) return;
    try {
      // Fetch rooms
      final roomsResponse = await Supabase.instance.client.from('rooms').select();
      final fetchedRooms = List<Room>.from(roomsResponse.map((data) => Room.fromJson(data)));
      _rooms = fetchedRooms; // Update the _rooms list

      // Fetch existing devices for the user
      final devicesResponse = await Supabase.instance.client
          .from('devices')
          .select()
          .eq('user_id', user.id);
      final existingDevices = List<Map<String, dynamic>>.from(devicesResponse);

      // Initialize roomDevices with existing devices mapped to rooms
      _roomDevices = {
        for (final room in _rooms)
          room.id: existingDevices.where((device) => device['room_id'] == room.id).toList()
      };

      // Initialize TabController after fetching rooms
      ...
      setState(() {
        // Trigger a rebuild after data is fetched and state is updated
      });
              ...
              children: [
                ...
                Padding(
                  ...
                  child: TabBar(
                    ...
                    indicatorWeight: 3,
                    tabs: _rooms.map((room) => Tab(text: room.name)).toList(),
                  ),
                ),
                                          ...
                                          child: Row(
                                            ...
                                            children: [
                                              ...
                                              IconButton(
                                                ...
                                                onPressed: () {
                                                  setState(() {
                                                    _roomDevices[room.id]!.removeAt(index);
                                                  });
                                                },
                                              ),
                                            ],
                                  ...
                                  child: ElevatedButton.icon(
                                    onPressed: () async {
                                      ...
                                      if (connectedPlugs != null) {
                                        setState(() {
                                          _roomDevices[room.id]!.addAll(connectedPlugs);
                                        });
                                      }
                                    },
                                    icon: const Icon(Icons.add_circle_outline, color: Colors.white, size: 20),
