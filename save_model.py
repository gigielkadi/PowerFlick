...
# Save the model
model.save("AI_model/backend/trained_model.h5")
print("Model saved to AI_model/backend/trained_model.h5")

# Test a prediction
print("\nTesting prediction...")
sample_input = X_test[:1]  # Take first test sample
prediction = model.predict(sample_input)
actual = y_test[:1]
print(f"Predicted: {prediction[0]:.4f}")
print(f"Actual: {actual[0]:.4f}")
